...
if __name__ == '__main__':
    # 加载模型
    model = YOLO(r'the path to the SegS-Yolo.yaml file on your computer', task="segment")  # 不使用预训练权重训练
    # model = YOLO(r'yolov8-seg.yaml').load("yolov8n-seg.pt")  # 使用预训练权重训练
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data=r'the path to the“\ultralytics-main\ultralytics\cfg\datasets\coco8-seg.yaml” on your computer',
        ...
        project='runs/train/SegS-Yolo',  # (str, optional) 项目名称
