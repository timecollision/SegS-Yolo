...
class Results(SimpleClass):
    ...
    def plot(
        ...
        if pred_masks and show_masks:
            ...
            annotator.masks(pred_masks.data, colors=[colors(x, True) for x in idx], im_gpu=im_gpu)
            ####分析偏差原因 ：在绘制掩码时，annotator.masks函数内部调用了ops.scale_coords进行了尺度变换，而在绘制质心和长轴时没有进行这步操作
            """
            reshaped_pred_masks = pred_masks.data.unsqueeze(1).reshape(pred_masks.data.shape[0], 1, pred_masks.data.shape[1], pred_masks.data.shape[2])
            
            scaled_tensor = ops.scale_image(reshaped_pred_masks, self.orig_shape)
            scaled_masks = Masks(scaled_tensor, self.orig_shape)
            centroid_and_axis_list = scaled_masks.get_centroid_and_axis()
            
            
            centroid_and_axis_list = pred_masks.get_centroid_and_axis()
            #for i, (centroid, axis_max, axis_min) in enumerate(centroid_and_axis_list):
            for i, centroid in enumerate(centroid_and_axis_list):
                # 使用 box_label 方法模拟绘制带有颜色的矩形
                # annotator.box_label([centroid[0], centroid[1], centroid[0] + axis[0] * 10000, centroid[1] + axis[1] * 10000], label=str(i), color=colors(i, True))
                # 使用 annotator 绘制质心
                # 绘制质心
                annotator.centroid_masks(centroid, colors=[colors(x, True) for x in idx], im_gpu=im_gpu)
                
                
                # 绘制长轴方向向量
                length_factor = 50
                end_point = (int(centroid[0] + axis_max[0] * length_factor), int(centroid[1] + axis_max[1] * length_factor))
                annotator.line(centroid, end_point, color=colors(0), vector_length=10)

                #
                length_factor = 50
                end_point = (int(centroid[0] + axis_min[0] * length_factor), int(centroid[1] + axis_min[1] * length_factor))
                annotator.line(centroid, end_point, color=colors(0), vector_length=10)
            ...
            centroid_and_axis_list = pred_masks.get_centroid_and_axis()
            for i, (centroid, axis_max, axis_min) in enumerate(centroid_and_axis_list):
                # 使用 LetterBox 处理图像，并根据处理后的图像尺寸调整质心坐标
                adjusted_centroid = None
                if im_gpu is not None:
                    processed_img = LetterBox(self.orig_img.shape[:2])(image=annotator.result())
                    ...
                    adjusted_centroid = [centroid[0] * x_scale, centroid[1] * y_scale]
                else:
                    adjusted_centroid = centroid

                # 绘制质心
                if adjusted_centroid is not None:
                    annotator.circle(adjusted_centroid, radius=5, color=colors(0))

                # 绘制长轴方向向量
                length_factor = 50
                end_point_max = (
                int(centroid[0] + axis_max[0] * length_factor), int(centroid[1] + axis_max[1] * length_factor))
                # 使用 LetterBox 处理图像，并根据处理后的图像尺寸调整长轴端点坐标
                adjusted_end_point_max = None
                if im_gpu is None:
                    adjusted_end_point_max = [end_point_max[0] * x_scale, end_point_max[1] * y_scale]
                if adjusted_end_point_max is not None and adjusted_centroid is not None:
                    annotator.line(adjusted_centroid, adjusted_end_point_max, color=colors(0), vector_length=10)

                # 绘制短轴方向向量
                length_factor = 50
                end_point_min = (
                int(centroid[0] + axis_min[0] * length_factor), int(centroid[1] + axis_min[1] * length_factor))
                # 使用 LetterBox 处理图像，并根据处理后的图像尺寸调整短轴端点坐标
                adjusted_end_point_min = None
                if im_gpu is None:
                    adjusted_end_point_min = [end_point_min[0] * x_scale, end_point_min[1] * y_scale]
                if adjusted_end_point_min is not None and adjusted_centroid is not None:
                    annotator.line(adjusted_centroid, adjusted_end_point_min, color=colors(0), vector_length=10)
            """
        # Plot Detect results
...
class Masks(BaseTensor):
    ...
    def get_centroid_and_axis(self):
        """
        计算分割掩码的质心和特征分解的长轴。

        Returns:
            list: 包含每个分割掩码的质心和长轴信息的列表。
                每个元素是一个元组，包含质心坐标和长轴向量。
        """
        centroid_and_axis_list = []
        masks = self.data
        for mask in masks:
            # 这里假设 mask 是一个二维的 numpy 数组，表示分割掩码
            points = np.argwhere(mask > 0)
            if isinstance(points, torch.Tensor):
                points = points.numpy()
            if points.size > 0:
                centroid = np.mean(points, axis=0)
                #ops.scale_coords(self.data.shape[1:], centroid, self.orig_shape, normalize=False)
                ...
                #centroid_and_axis_list.append((centroid, major_axis, minor_axis))
                centroid_and_axis_list.append(centroid)
            else:
                #centroid_and_axis_list.append((None, None, None))
                centroid_and_axis_list.append(None)
        return centroid_and_axis_list
        """
        # 对每个掩码进行尺度变换并转换为段表示
        transformed_masks = []
        for mask in masks:
            img1_shape = mask.shape
            img0_shape = (new_height, new_width)  # 这里需要你提供新的图像形状
            coords = torch.from_numpy(np.argwhere(mask > 0)).float()
            transformed_coords = scale_coords(img1_shape, coords, img0_shape)
            transformed_mask = masks2segments(torch.from_numpy(mask).unsqueeze(0), strategy="largest")[0]
            transformed_masks.append(transformed_mask)

        for transformed_mask in transformed_masks:
            if transformed_mask.size > 0:
                centroid = np.mean(transformed_mask, axis=0)
                covariance_matrix = np.cov(transformed_mask.T)
                eigenvalues, eigenvectors = np.linalg.eig(covariance_matrix)
                major_axis = eigenvectors[:, np.argmax(eigenvalues)]
                minor_axis = eigenvectors[:, np.argmin(eigenvalues)]
                centroid_and_axis_list.append((centroid, major_axis, minor_axis))
