...
class Annotator:
    ...
    # 添加 circle 方法用于绘制圆形
    def circle(self, center, radius=5, color=(255, 42, 4)):
        if self.pil:
    ...
    def line(self, start_point, end_point, color=(255, 0, 0), vector_length=None):
        if self.pil:
            draw = ImageDraw.Draw(self.im)
            if vector_length is not None:
                end_point = (int(start_point[0] + (end_point[0] - start_point[0]) * vector_length),
                             int(start_point[1] + (end_point[1] - start_point[1]) * vector_length))
            draw.line([(start_point[0], start_point[1]), (end_point[0], end_point[1])], fill=color, width=2)
        else:
            if vector_length is not None:
                end_point = (int(start_point[0] + (end_point[0] - start_point[0]) * vector_length),
                             int(start_point[1] + (end_point[1] - start_point[1]) * vector_length))
            cv2.line(self.im, (int(start_point[0]), int(start_point[1])), (int(end_point[0]), int(end_point[1])), color, 2)
    ...
    def centroid_masks(self, masks, colors, im_gpu, alpha=1, retina_masks=False):
        """
        Plot centroid masks on image.
